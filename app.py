import sys
sys.path.append('/app/yolov5/') # Adjust here
...
checkpoint_path = '/app/epoch=47-step=668-val_accuracy=100.0000-val_NED=100.0000.ckpt'
...
def process_images(image_path_1, image_path_2,model_ocr):
    ...
    ocr_results_left = process_detections(detections_left,image_path_1,model_ocr, model_args)
    ocr_results_right = process_detections(detections_right,image_path_2,model_ocr, model_args)
    marked_left_image = draw_detections_with_text(cv2.imread(image_path_1),detections_left,
                                                  [ocr_results_left['DotPeenText'][0],ocr_results_left['MachinePeenText'][0]])
    marked_right_image = draw_detections_with_text(cv2.imread(image_path_2),detections_right,
                                                  [ocr_results_right['DotPeenText'][0],ocr_results_right['MachinePeenText'][0]])
...
def main():
    ...
    if st.button("Submit"):
        if uploaded_image1 and uploaded_image2:
            ...
            ocr_results_left,marked_left_image,ocr_results_right,marked_right_image = process_images('/app/left-image.jpg', '/app/right-image.jpg',model_ocr)
            ...
            with col1:
                ...
                st.markdown(f"**MachinePeenText:** **{ ocr_results_left['MachinePeenText'][0]}** {ocr_results_left['MachinePeenText'][1]}")
                st.markdown(f"**DotPeenText:** **{ ocr_results_left['DotPeenText'][0]}** {ocr_results_left['DotPeenText'][1]}")
            
            # Column 2
            with col2:
                st.image(marked_right_image, caption="Right Workstation Processed Image", use_column_width=True)
                st.markdown(f"**MachinePeenText:** **{ ocr_results_right['MachinePeenText'][0]}** {ocr_results_right['MachinePeenText'][1]}")
                st.markdown(f"**DotPeenText:** **{ ocr_results_right['DotPeenText'][0]}** {ocr_results_right['DotPeenText'][1]}")
        
        else:
            st.warning("Please upload both images.")
        if (ocr_results_right['MachinePeenText'][0] == ocr_results_left['MachinePeenText'][0]) and (ocr_results_right['DotPeenText'][0] == ocr_results_left['DotPeenText'][0]) :
